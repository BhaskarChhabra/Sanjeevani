...
import { loginUser, resendOtp } from '../api'; 
...
// --- Custom Input ---
const CustomInput = ({ label, name, type, value, onChange, placeholder, required, showForgot, showEye, onForgotClick }) => {
        ...
        <div style={{ marginBottom: '1.25rem', textAlign: 'left', position: 'relative' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                ...
                {showForgot && (
                    <a 
                        href="#"
                        onClick={(e) => { e.preventDefault(); onForgotClick && onForgotClick(); }}
                        style={{ color: COLOR_ACCENT_LINK, fontSize: '0.875rem', textDecoration: 'none' }}
                    >
                        Forgot password?
                    </a>
                )}
            ...
            <div style={{ position: 'relative' }}>
                ...
                {showEye && (
                    <span 
                        style={{ position: 'absolute', right: '12px', top: '12px', color: COLOR_SUBTLE_TEXT, cursor: 'pointer', userSelect: 'none' }}
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <form onSubmit={handleSubmit} style={{ width: '100%', display: "flex", flexDirection: "column", gap: '0.5rem' }}>
                    ...
                    {showVerifyOption && (
                        <button
                            ...
                            style={{ marginTop: '1rem', background: 'transparent', color: COLOR_ACCENT_LINK, border: 'none', cursor: 'pointer', fontWeight: 700 }}
                        >
                ...
                <div style={{ marginTop: '2rem', textAlign: 'center', fontSize: '0.9rem', color: COLOR_SUBTLE_TEXT, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: '1rem' }}>
                    Don't have an account?{" "}
                    <Link to="/register" style={{ color: COLOR_ACCENT_LINK, fontWeight: '700', textDecoration: 'none' }}>Sign up</Link>
                </div>
            </div>
        </div>
    );
